...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-40 hidden items-center justify-center">
    <div class="bg-white p-6 rounded-xl shadow-lg w-96">
        ...
        <form id="editPersonnelForm">
            ...
            <input type="text" name="name" class="w-full mb-2 border p-2 rounded" required>
            <input type="email" name="email" class="w-full mb-2 border p-2 rounded" required>
            <select name="role" class="w-full mb-2 border p-2 rounded" required>
                <option value="">Select Role</option>
                <?php foreach($roles as $r): ?>
                    <option value="<?= $r ?>"><?= $r ?></option>
            ...
            <input type="number" step="0.01" name="rate" class="w-full mb-2 border p-2 rounded" required>
            <input type="text" name="category" class="w-full mb-4 border p-2 rounded" required>
            <div class="flex justify-end gap-2">
...
<?php renderLayout("Personnel", ob_get_clean(), "personnel"); ?>
